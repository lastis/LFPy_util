...
//   var angularSpeed = 0.01;
  ...
  return {
    drawNeuron: function() {
      ...
      loadNeuronMesh();
      loadElectrodesMesh();
      loadContours('contour_dir_list.js');
    }
  }
  ...
  function animate(){
    ...
    if (contoursLoaded && frame_dt_counter >= frame_dt){
      frame_dt_counter = 0;
      for(i = 0; i < n_contours; i++){
        scene.remove(contours[i][frame_index]);
      }
      frame_index++;
      ...
      for(i = 0; i < n_contours; i++){
        scene.add(contours[i][frame_index]);
      }
    }
  ...
  function loadContours(dir_list_js) {
    var read_contours = false;
    var read_frames = false;
    contours = [];

    var json = $.getJSON('contour_dir_list.js', readDirList);
    var json = $.getJSON('contour_0000/config.js', readFileList);

    function readDirList(data){
      n_contours = data.n_contours;
      read_contours = true;
      if (read_frames){
        loadAllContours();
      }
    ...
    function loadAllContours(){
      var total_files = n_frames*n_contours;
      var file_cnt = 0;
      console.log(total_files+' contour files found.');
      for (i = 0; i < n_contours; i++){
        contours[i] = [];
        for (j = 0; j < n_frames; j++){
          (function(i,j){
            contours[i][j] = [];
            file_path = 'contour_'+pad(i,4)+'/frame_'+pad(j,4)+'.js';
            loader.load(file_path,function(geo){
              contours[i][j] = new THREE.Mesh(geo, new THREE.MeshNormalMaterial());
              file_cnt += 1;
              if (file_cnt == total_files){
                contoursLoaded = true;
                draw();
              }
            });
