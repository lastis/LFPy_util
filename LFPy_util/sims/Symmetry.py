...
class Symmetry(Simulation):
    ...
    def process_data(self):
        ...
        widths_I, widths_I_trace = de.find_wave_width_type_I(spikes, 
                dt=data['dt'])
        widths_II, widths_II_trace = de.find_wave_width_type_II(spikes, 
                dt=data['dt'], amp_option=self.amp_option)
        ...
        data['r_vec'] = np.linspace(run_param['R_0'],run_param['R'],run_param['n'])
    ...
    def plot(self,dir_plot):
        ...
        for t in xrange(data['amps_mean'].shape[0]):
            ...
            plt.plot(data['r_vec'], data['amps_mean'][t],
                    color=c[t],marker='o',markersize=5,label=label)
            ax.fill_between(
                    data['r_vec'],
        ...
        for t in xrange(data['widths_I_mean'].shape[0]):
            label = r'$\theta = {}\degree$'.format(run_param['theta'][t])
            plt.plot(data['r_vec'], data['widths_I_mean'][t],
                    color=c[t],marker='o',markersize=5,label=label)
            ax.fill_between(
                    data['r_vec'],
        ...
        for t in xrange(data['widths_II_mean'].shape[0]):
            label = r'$\theta = {}\degree$'.format(run_param['theta'][t])
            plt.plot(data['r_vec'], data['widths_II_mean'][t],
                    color=c[t],marker='o',markersize=5,label=label)
            ax.fill_between(
                    data['r_vec'],
        ...
        if self.plot_detailed:
            ...
            for i in xrange(t):
                for j in xrange(p):
                    for k in xrange(n):
                        fname = 'sym_elec_t_{}_p_{}_n_{}'.format(run_param['theta'][i],j*360/p,k)
                        print "plotting            :", fname
                        ...
                        lplot.nice_axes(ax)
                        # Plot
                        plt.plot(data['spikes_t_vec'], data['spikes'][cnt],color=c[0])

                        # Trace I
                        trace_idx = np.where(~np.isnan(data['widths_I_trace'][cnt]))[0]
                        trace_idx = [trace_idx[0],trace_idx[-1]]
                        plt.plot(data['spikes_t_vec'], data['widths_I_trace'][cnt],
                                color=c[1])
                        plt.plot(data['spikes_t_vec'][trace_idx], 
                                data['widths_I_trace'][i][trace_idx],
                                color=c[1], marker="|")
                        # Trace II
                        trace_idx = np.where(~np.isnan(data['widths_II_trace'][cnt]))[0]
                        trace_idx = [trace_idx[0],trace_idx[-1]]
                        plt.plot(data['spikes_t_vec'], data['widths_II_trace'][cnt],
                                color=c[1])
                        # Save plt.
